...
public class OozieJobDaoImpl extends OozieJobDao {
    ...
    public List<OozieJobRecord> lastNOozieJobs(int n) {
        ...
        return n == 1 ?
                Collections.singletonList(jdbcTemplate.query(SELECT_LAST_N_JOB, tableDefinition.getResultSetExtractor())) :
    ...
    public Optional<OozieJobRecord> lastOozieJobWithStatus(WorkflowJob.Status status) {
        ...
        return Optional.ofNullable(jdbcTemplate.query(SELECT_LAST_JOB_WITH_STATUS, tableDefinition.getResultSetExtractor()));
    }
}
